...
def login():
    errors = {}
    success = request.args.get('success')  # Receber a mensagem de sucesso da URL
    ...
    if request.method == 'POST':
        ...
        if conn:
            ...
            if user:
                user_id, nome, user_matricula, user_senha_hashed, tipo, data_cadastro = user
                if check_password_hash(user_senha_hashed, senha):
                    session['user_id'] = user_id
                    session['nome'] = nome
                    session['tipo'] = tipo
                    session['matricula'] = user_matricula  # Adiciona a matrícula à sessão
                    if tipo == 'admin':
                ...
                errors['matricula'] = 'Matrícula inexistente.'
            conn.close()
        else:
            errors['login'] = 'Erro ao conectar ao banco de dados.'
    ...
    if request.method == 'POST':
        ...
        try:
            c.execute("""
                ...
                WHERE matricula_usuario = (SELECT matricula FROM usuarios WHERE id = ?) AND data = ?
        ...
        c.execute("SELECT data FROM pontos WHERE matricula_usuario = (SELECT matricula FROM usuarios WHERE id = ?) ORDER BY data DESC", (usuario_id,))
        datas = c.fetchall()
        ...
        c.execute("SELECT * FROM pontos WHERE matricula_usuario = (SELECT matricula FROM usuarios WHERE id = ?) AND data = ?", (usuario_id, data_selecionada))
        ponto = c.fetchone()
...
def editar_usuario(id):
    ...
    c = conn.cursor()
    if request.method == 'POST':
        c.execute("SELECT nome, matricula, tipo FROM usuarios WHERE id = ?", (id,))
        ...
        senha = request.form.get('senha', '').strip()
        tipo = request.form.get('tipo', '').strip() or usuario_atual[2]

        campos_atualizados = []

        try:
            # Verificar quais campos foram modificados
            if nome != usuario_atual[0]:
                c.execute("UPDATE usuarios SET nome = ? WHERE id = ?", (nome, id))
                campos_atualizados.append(f'nome de {usuario_atual[0]} para {nome}')
            if matricula != usuario_atual[1]:
                c.execute("UPDATE usuarios SET matricula = ? WHERE id = ?", (matricula, id))
                campos_atualizados.append(f'matrícula de {usuario_atual[1]} para {matricula}')
            if senha:
                senha_hashed = generate_password_hash(senha)
                c.execute("UPDATE usuarios SET senha = ? WHERE id = ?", (senha_hashed, id))
                campos_atualizados.append('senha')
            if tipo != usuario_atual[2]:
                c.execute("UPDATE usuarios SET tipo = ? WHERE id = ?", (tipo, id))
                campos_atualizados.append(f'tipo de {usuario_atual[2]} para {tipo}')

            conn.commit()

            if campos_atualizados:
                campos_str = ', '.join(campos_atualizados)
                flash(f'Alterações feitas com sucesso! Campos alterados: {campos_str}', 'success')
            else:
